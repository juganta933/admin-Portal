<script>
  ...
  const handleFormSubmit = ({ formData, cancel }) => {
    ...
    if (!isEditing && (!imageFile || imageFile.size === 0)) {
      clientError = "Please upload a project image.";
      cancel();
      return;
    }
    ...
    if (!live) {
      clientError = "Live project URL is required.";
      cancel();
      return;
    }
  ...
  const categoryAccents = {
    ...
    "Web Development": "#00FFD1",
    "Deep Learning": "#8B5CF6",
    "Artificial Intelligence": "#FFBD2E",
    "Data Science": "#F472B6"
  };
  ...
  function resetForm() {
    projectForm.title = "";
    projectForm.category = "Machine Learning";
  ...
  function openEditModal(project) {
    ...
    projectForm.techStack = Array.isArray(project.techStack)
  ...
  function showTechStack(techStack) {
...
<svelte:head>
  <title>Projects | ML Club Admin</title>
...
>
  ...
  <div class="pointer-events-none absolute inset-0">
    ...
    <div class="absolute inset-0 project-grid opacity-50"></div>
  ...
  <div class="relative z-10 mx-auto max-w-7xl">
    ...
    <div class="page-enter flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p class="mt-5 max-w-2xl text-[15px] leading-8 text-slate-400">
          Add, edit, and organize ML Club projects with showcase images, tech stacks, GitHub repositories, and live deployments.
        </p>
      </div>
    ...
    <div class="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
      {#each categories as category, index}
        <div
          ...
          style={`--accent:${category.accent}; animation-delay:${0.12 + index * 0.07}s;`}
        >
    ...
    <div class="mt-12 grid gap-7 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div class="empty-card col-span-full overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#070A16]/80 p-[1px] shadow-[0_30px_100px_rgba(0,0,0,0.65)] backdrop-blur-2xl">
          <div class="rounded-[27px] bg-[linear-gradient(180deg,rgba(10,13,28,0.96),rgba(2,3,11,0.98))] p-10 text-center">
            ...
            <p class="mt-3 text-sm leading-7 text-slate-500">
              Click Add Project to create your first ML Club project showcase.
            </p>
          </div>
        </div>
      {/if}
      ...
      {#each projects as project, index}
        <div
          class="project-card group relative overflow-hidden rounded-[30px] border border-white/[0.075] bg-[#070A16]/85 p-[1px] shadow-[0_28px_95px_rgba(0,0,0,0.60)] backdrop-blur-2xl transition duration-500 hover:-translate-y-3 hover:border-white/15 hover:shadow-[0_42px_135px_rgba(0,0,0,0.82)]"
          style={`--accent:${categoryAccents[project.category] || "#FF4D00"}; animation-delay:${0.25 + index * 0.06}s;`}
        >
          ...
          <div class="relative rounded-[29px] bg-[linear-gradient(180deg,rgba(10,13,28,0.96),rgba(2,3,11,0.98))] p-4">
            ...
            <div class="relative overflow-hidden rounded-[24px] border border-white/[0.07] bg-black/35">
              ...
              <div class="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 transition duration-500 group-hover:translate-y-0 group-hover:opacity-100">
            ...
            <div class="px-1 pb-1 pt-5">
              ...
              <div class="mt-5">
                <p class="font-mono text-[9px] uppercase tracking-[0.15em] text-slate-600">
                  Tech Stack
                </p>

                <div class="mt-2 flex flex-wrap gap-2">
                  {#if techList(project.techStack).length > 0}
                    {#each techList(project.techStack).slice(0, 5) as tech}
                      <span class="rounded-full border border-white/[0.08] bg-white/[0.035] px-3 py-1 font-mono text-[9px] font-bold uppercase tracking-[0.10em] text-slate-400 transition group-hover:border-[var(--accent)] group-hover:text-white">
                        {tech}
                      </span>
                    {/each}
                  {:else}
                    <span class="rounded-full border border-white/[0.08] bg-white/[0.035] px-3 py-1 font-mono text-[9px] font-bold uppercase tracking-[0.10em] text-slate-500">
                      Not specified
                    </span>
                  {/if}
                </div>
    ...
    <div class="modal-card relative w-full max-w-4xl overflow-hidden rounded-[30px] border border-white/[0.08] bg-[#050712]/95 p-[1px] shadow-[0_40px_140px_rgba(0,0,0,0.90)]">
      ...
      <div class="relative rounded-[29px] bg-[linear-gradient(180deg,rgba(10,13,28,0.98),rgba(2,3,11,0.99))] p-6 sm:p-8">
        ...
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-2 text-sm leading-6 text-slate-500">
              Fill the project details below. Image upload supports JPG, PNG, and WEBP.
            </p>
          </div>
        ...
        <form
          ...
          action={isEditing ? "?/updateProject" : "?/addProject"}
        ...
        >
          ...
          {#if isEditing}
            ...
            <input type="hidden" name="oldImage" value={projectForm.image} />
          {/if}
          ...
          <div>
            ...
            <input
              name="techStack"
              bind:value={projectForm.techStack}
              type="text"
              placeholder="Example: SvelteKit, MongoDB, Tailwind"
              class="form-input"
            />
          </div>
          ...
          <div class="md:col-span-2">
            ...
            <input type="hidden" name="category" value={projectForm.category} />

            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {#each categoryOptions as category}
                <button
                  type="button"
                  onclick={() => (projectForm.category = category)}
    ...
    <div class="delete-card relative w-full max-w-md overflow-hidden rounded-[30px] border border-red-500/20 bg-[#050712]/95 p-[1px] shadow-[0_40px_140px_rgba(0,0,0,0.90)]">
      ...
      <div class="relative rounded-[29px] bg-[linear-gradient(180deg,rgba(20,7,10,0.96),rgba(2,3,11,0.99))] p-6">
        ...
        <p class="mt-5 text-sm leading-7 text-slate-400">
          ...
          <span class="font-bold text-orange-300">{deletingProject.title}</span>?
        ...
        <div class="mt-7 flex justify-end gap-3">
          ...
          <form method="POST" action="?/deleteProject">
            <input type="hidden" name="id" value={deletingProject._id} />
...
<style>
  ...
  .project-card:hover .project-aura {
    opacity: 0.68;
  }
