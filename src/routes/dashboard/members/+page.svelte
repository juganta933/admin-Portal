<script>
  ...
  const handleFormSubmit = ({ formData, cancel }) => {
    ...
    if (!isEditing && (!imageFile || imageFile.size === 0)) {
      clientError = "Please upload a member image.";
      cancel();
      return;
    }
    ...
    if (!linkedin) {
      clientError = "LinkedIn URL is required.";
      cancel();
      return;
    }

    if (!github) {
      clientError = "GitHub URL is required.";
      cancel();
      return;
    }
  ...
  const roleAccents = {
    Executive: "#FF4D00",
    "ML Core Head": "#00FFD1",
    "Web Developer Head": "#8B5CF6",
    "PR and Marketing": "#FFBD2E",
    "Design and Video Editing": "#F472B6",
    "Junior ML core Member":"#38BDF8",
     "Junior Web Developer":"#22C55E"
  };
  ...
  let roles = $derived(
    roleOptions.map((role) => ({
      ...
      accent: roleAccents[role] || "#FF4D00"
    }))
  );

  function resetForm() {
    memberForm.name = "";
    memberForm.role = "Executive";
  ...
  function openEditModal(member) {
    ...
    memberForm.department = member.department;
...
<svelte:head>
  <title>Members | ML Club Admin</title>
...
>
  ...
  <div class="pointer-events-none absolute inset-0">
    ...
    <div class="absolute inset-0 member-grid opacity-50"></div>
  ...
  <div class="relative z-10 mx-auto max-w-7xl">
    ...
    <div class="page-enter flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p class="mt-5 max-w-2xl text-[15px] leading-8 text-slate-400">
          Add, edit, and remove ML Club members with profile images, roles, departments, and verified social links.
        </p>
      </div>
    ...
    <div class="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
      {#each roles as role, index}
        <div
          class="role-card group relative overflow-hidden rounded-[24px] border border-white/[0.075] bg-[#070A16]/80 p-[1px] shadow-[0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl transition duration-500 hover:-translate-y-2 hover:border-white/15 hover:shadow-[0_34px_110px_rgba(0,0,0,0.75)]"
          style={`--accent:${role.accent}; animation-delay:${0.12 + index * 0.07}s;`}
        >
          ...
          <div class="relative rounded-[23px] bg-[linear-gradient(180deg,rgba(10,13,28,0.94),rgba(3,5,13,0.96))] p-5">
            <div class="flex items-center justify-between gap-3">
              ...
              <div class="h-2 w-2 rounded-full bg-[var(--accent)] shadow-[0_0_18px_var(--accent)]"></div>
            </div>
    ...
    <div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <div class="empty-card col-span-full overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#070A16]/80 p-[1px] shadow-[0_30px_100px_rgba(0,0,0,0.65)] backdrop-blur-2xl">
          <div class="rounded-[27px] bg-[linear-gradient(180deg,rgba(10,13,28,0.96),rgba(2,3,11,0.98))] p-10 text-center">
            ...
            <p class="mt-3 text-sm leading-7 text-slate-500">
              Click Add Member to create your first ML Club member profile.
            </p>
          </div>
        </div>
      {/if}
      ...
      {#each members as member, index}
        <div
          class="member-card group relative overflow-hidden rounded-[28px] border border-white/[0.075] bg-[#070A16]/85 p-[1px] shadow-[0_28px_95px_rgba(0,0,0,0.60)] backdrop-blur-2xl transition duration-500 hover:-translate-y-3 hover:border-white/15 hover:shadow-[0_42px_135px_rgba(0,0,0,0.82)]"
          style={`--accent:${roleAccents[member.role] || "#FF4D00"}; animation-delay:${0.25 + index * 0.06}s;`}
        >
          <div class="member-aura"></div>

          <div class="relative rounded-[27px] bg-[linear-gradient(180deg,rgba(10,13,28,0.96),rgba(2,3,11,0.98))] p-4">
            ...
            <div class="relative overflow-hidden rounded-[22px] border border-white/[0.07] bg-black/35">
              <img
                src={member.image ||
                  "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=500&q=80"}
                alt={member.name}
                class="h-64 w-full object-cover transition duration-700 group-hover:scale-110 group-hover:rotate-1"
              />
              ...
              <span class="absolute left-3 top-3 rounded-full border border-white/10 bg-black/55 px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-[var(--accent)] backdrop-blur-xl shadow-[0_0_28px_color-mix(in_srgb,var(--accent)_20%,transparent)]">
                {member.role}
              </span>

              <div class="absolute bottom-3 left-3 right-3 translate-y-4 opacity-0 transition duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                <div class="rounded-2xl border border-white/10 bg-black/50 p-3 backdrop-blur-xl">
            ...
            <div class="px-1 pb-1 pt-5">
              <h2 class="text-xl font-extrabold tracking-[-0.04em] text-white transition duration-300 group-hover:text-[var(--accent)]">
                {member.name}
              </h2>
              ...
              <div class="mt-5 flex items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                  <span class="h-1.5 w-1.5 rounded-full bg-[var(--accent)] shadow-[0_0_12px_var(--accent)]"></span>
...
{#if showModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4 backdrop-blur-md">
    <div class="modal-card relative w-full max-w-3xl overflow-hidden rounded-[30px] border border-white/[0.08] bg-[#050712]/95 p-[1px] shadow-[0_40px_140px_rgba(0,0,0,0.90)]">
      ...
      <div class="relative rounded-[29px] bg-[linear-gradient(180deg,rgba(10,13,28,0.98),rgba(2,3,11,0.99))] p-6 sm:p-8">
        ...
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-2 text-sm leading-6 text-slate-500">
              Fill the member details below. Image upload supports JPG, PNG, and WEBP.
            </p>
          </div>
        ...
        <form
          ...
          action={isEditing ? "?/updateMember" : "?/addMember"}
        ...
        >
          ...
          {#if isEditing}
            ...
            <input type="hidden" name="oldImage" value={memberForm.image} />
          {/if}
          ...
          <div>
            ...
            <input
              name="department"
              bind:value={memberForm.department}
              type="text"
          ...
          <div class="md:col-span-2">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {#each roleOptions as role}
                <button
                  type="button"
                  onclick={() => (memberForm.role = role)}
                  class={`role-picker group relative overflow-hidden rounded-2xl border px-4 py-3 text-left transition duration-300 ${
                ...
                >
                  ...
                  <span class="relative flex items-center gap-3">
                    <span
                      ...
                      style={`background:${roleAccents[role] || "#FF4D00"}; color:${roleAccents[role] || "#FF4D00"};`}
                    ></span>
    ...
    <div class="delete-card relative w-full max-w-md overflow-hidden rounded-[30px] border border-red-500/20 bg-[#050712]/95 p-[1px] shadow-[0_40px_140px_rgba(0,0,0,0.90)]">
      ...
      <div class="relative rounded-[29px] bg-[linear-gradient(180deg,rgba(20,7,10,0.96),rgba(2,3,11,0.99))] p-6">
        ...
        <p class="mt-5 text-sm leading-7 text-slate-400">
          ...
          <span class="font-bold text-orange-300">{deletingMember.name}</span>?
        ...
        <div class="mt-7 flex justify-end gap-3">
          ...
          <form method="POST" action="?/deleteMember">
            <input type="hidden" name="id" value={deletingMember._id} />
